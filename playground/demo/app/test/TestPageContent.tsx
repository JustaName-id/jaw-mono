...
import { JAW, ProviderInterface, Mode } from '@jaw.id/core';
import { JAWProvider, useJAWProvider } from '@jaw/ui';
...
const STORAGE_KEY = 'jaw-test-mode';

function getStoredMode(): SDKMode {
  if (typeof window === 'undefined') return 'cross-platform';
  return (localStorage.getItem(STORAGE_KEY) as SDKMode) || 'cross-platform';
}

function setStoredMode(mode: SDKMode) {
  localStorage.setItem(STORAGE_KEY, mode);
}

// Clear SDK's stored signer type to prevent mode conflicts
function clearSDKSignerType() {
  localStorage.removeItem('JAWSDK:SignerConfig:SignerType');
}

// Context for cross-platform mode
const CrossPlatformContext = createContext<ProviderInterface | null>(null);

function CrossPlatformWrapper({ children }: { children: React.ReactNode }) {
  const [provider, setProvider] = useState<ProviderInterface | null>(null);

  useEffect(() => {
    // Clear any stored app-specific signer type before creating the cross-platform SDK
    clearSDKSignerType();
  ...
  if (!provider) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
        <p className="text-gray-600 dark:text-gray-400">Initializing SDK...</p>
      </div>
    );
  }

  return (
    <CrossPlatformContext.Provider value={provider}>
      {children}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex rounded-lg overflow-hidden border border-gray-300 dark:border-gray-600">
            <button
              onClick={() => onModeChange('cross-platform')}
            ...
            <button
              onClick={() => onModeChange('app-specific')}
              className={`px-4 py-2 text-sm font-medium transition-colors ${
                mode === 'app-specific'
                  ? 'bg-blue-600 text-white'
                  : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600'
      ...
      <div className="mt-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
        {mode === 'cross-platform' ? (
          <p className="text-sm text-gray-600 dark:text-gray-400">
            <strong>Cross-Platform Mode:</strong> Opens authentication dialogs in an external popup window.
            Uses keysUrl configuration for wallet communication.
          </p>
        ) : (
          <p className="text-sm text-gray-600 dark:text-gray-400">
            <strong>App-Specific Mode:</strong> Renders authentication dialogs directly in-app using React portals.
            Passkey authentication happens within your application.
          </p>
        )}
      </div>
    </div>
...
// Wrapper for app-specific mode that calls useJAWProvider unconditionally
function AppSpecificTestContent({ mode }: { mode: SDKMode }) {
  const provider = useJAWProvider();
  return <TestContentInner provider={provider} mode={mode} />;
}

// Wrapper for cross-platform mode that uses context unconditionally
function CrossPlatformTestContent({ mode }: { mode: SDKMode }) {
  const provider = useContext(CrossPlatformContext)!;
  return <TestContentInner provider={provider} mode={mode} />;
}

// Test content component that receives provider as prop
function TestContentInner({ provider, mode }: { provider: ProviderInterface; mode: SDKMode }) {
        ...
        <div className="space-y-2">
          ...
          <p className="text-gray-700 dark:text-gray-300">
            ...
            <span className="text-blue-600">
              {mode === 'app-specific' ? 'App-Specific' : 'Cross-Platform'}
            </span>
          </p>
          {accounts.length > 0 && (
            <p className="text-gray-700 dark:text-gray-300">
...
// Main page export - handles mode selection and conditional rendering
export default function TestPageContent() {
  const [mode, setMode] = useState<SDKMode>('cross-platform');
  const [key, setKey] = useState(0);
  const [mounted, setMounted] = useState(false);

  // Load stored mode on mount and clear any stale SDK signer type
  useEffect(() => {
    clearSDKSignerType(); // Clear SDK's stored signer type on mount to ensure clean state
    setMode(getStoredMode());
    setMounted(true);
  }, []);

  const handleModeChange = (newMode: SDKMode) => {
    clearSDKSignerType(); // Clear SDK's stored signer type to prevent mode conflicts
    setStoredMode(newMode);
    setMode(newMode);
    setKey(k => k + 1); // Force re-initialization
  };
    ...
    <div className="min-h-screen p-8 bg-gray-50 dark:bg-gray-900">
      <div className="max-w-4xl mx-auto">
        ...
        {mode === 'app-specific' ? (
          ...
          >
            <AppSpecificTestContent key={`content-${key}`} mode={mode} />
          </JAWProvider>
        ) : (
          <CrossPlatformWrapper key={key}>
            <CrossPlatformTestContent key={`content-${key}`} mode={mode} />
