...
function usePhoneScale() {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const byHeight = (window.innerHeight - 140) / FRAME_H;
...
export function HeroDemoPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <section className="mx-auto w-full max-w-[1400px] px-9 pt-[22px] max-md:px-5">
      ...
      <main className="mx-auto grid w-full max-w-[1400px] flex-1 grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)] items-start max-[1040px]:grid-cols-1">
        <div className="flex min-h-0 flex-col justify-start py-[26px] pb-10 pl-9 pr-16 max-[1040px]:px-6 max-md:hidden">
        ...
        <div className="sticky top-5 mb-7 mr-7 mt-[26px] flex flex-col gap-4 max-[1040px]:static max-[1040px]:mx-6 max-[1040px]:mb-6 max-[1040px]:mt-0 max-md:mx-3">
          <div
            className="bg-raise flex flex-col items-center justify-center rounded-[28px] px-8 py-8 max-md:px-3 max-md:py-4"
            style={{
                ...
                <div className="origin-top-left" style={{ transform: `scale(${scale})` }}>
                  <IOSDevice width={PW} height={PH}>
                    <div className="animate-jd-fade relative h-full" key={`${id}-${v.key}`}>
                      ...
                      {/* mobile-only: feature switcher lives inside the phone */}
                      {!open && !fin && (
                        <button
                          type="button"
                          aria-label="Choose a feature"
                          onClick={() => setMenu(true)}
                          className="absolute right-4 top-[62px] z-[35] grid h-9 w-9 cursor-pointer place-items-center rounded-full border border-black/10 bg-white/75 shadow-[0_2px_10px_rgba(15,23,42,.12)] backdrop-blur-md md:hidden"
                        >
                          <svg
                            width="16"
                            height="16"
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center gap-2.5">
                                      <span
                                        className={`font-mono text-[11px] font-semibold ${on ? 'text-jaw-blue' : 'text-ink-3'}`}
                                      >
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 max-md:hidden">
            {fin && (
