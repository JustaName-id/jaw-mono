...
class Analytics {
  constructor() {
    if (typeof window !== 'undefined' && analyticsEnabled) {
      ...
      posthog.init(key, {
        ...
        // demo.jaw.id is a subdomain of jaw.id like every other JAW site.
        // Storing the distinct_id in a cookie scoped to the registrable domain
        // (.jaw.id) lets ONE anonymous identity follow a user across
        // landing/demo/docs/playground/dashboard — with the shared project key,
        // that is what makes the cross-app funnels work.
        persistence: 'localStorage+cookie',
        cross_subdomain_cookie: true,
        // Pageviews are tracked manually by AnalyticsProvider (the default only
        // fires on hard loads).
        capture_pageview: false,
        capture_pageleave: true,
        autocapture: true,
        // Only spawn person profiles once we identify() a connected account;
  ...
  identify(id: string) {
    ...
    // distinct_ids are case-sensitive — a checksummed (mixed-case) address
    // would fork one user into two person profiles.
