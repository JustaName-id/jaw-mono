...
2. Encrypts it to `~/.jaw/keystore.json` (AES-256-GCM, API-key-derived)
...
1. `jaw rpc call ... --session` decrypts the session key
...
The session key is scoped by **on-chain enforcement** — even if the key is compromised, damage is bounded:
...
| AES-256-GCM encryption     | File exfiltration (keystore useless without API key) |
