...
**An `upto` payment costs its whole ceiling until the chain confirms what it cost.** Once signed and sent, the authorization stays spendable up to the ceiling, and the receipt is the server's own claim about how much of it was taken. So the caps hold the ceiling for a failed payment and for a successful one nobody has checked yet. It is deliberately pessimistic: a run of failures would otherwise spend far past the cap.

The check happens on your next payment, and on `jaw x402 status`. It looks up the transaction the receipt named and reads the transfer in it, so the figure comes down to what actually moved. A slow node costs nothing: the row keeps its ceiling and is tried again.

`jaw x402 log` reports the same figure the caps enforce, so a row shows what is held rather than what was claimed.
...
A grant is refused when it asks for more than that allowance, or when it resets more often than that period, since the same allowance on a shorter period is more money over the same time. It is checked against the resolved permission, so a hand-written `--permissions` is bounded by it too. The ceiling is an amount of USDC, so with one set, a spend it cannot be priced against is refused rather than waved through: another token, or a chain with no USDC in the registry. Unset means no ceiling.

:::info
The ceiling only reaches grants made from this machine, so treat it as a convenience for terminal users rather than the control. An agent driving the wallet from a host with no terminal is not bounded by it. What stands in front of every grant either way is the browser approval, which is where the human signs.
:::

:::warning
These caps bound `jaw x402 pay` and `jaw_pay_and_fetch`. They are not a bound on the session itself. A session key can also be driven through `jaw_rpc`, and a `wallet_sendCalls` sent that way spends the permission directly: no cap here applies to it, and it leaves no ledger row. `jaw x402 status` reads the period figure off the chain when it can, so it still sees that spend; the session total is counted from the ledger alone and stays a floor.
...
USDC existing on a chain is not enough to list it. The permission manager has to be deployed there too, because a session pays through a permission and there is nothing to grant against otherwise. Polygon Amoy was listed on the token alone and has no manager, so it came off.
...
Once it passes a couple of megabytes, a payment folds its oldest rows into a single checkpoint row carrying their totals and moves the originals to `~/.jaw/x402-log.archive.jsonl`. Every figure the caps enforce comes out the same afterwards, because a fold only covers rows that every live cap either counts in full or ignores in full. A checkpoint shows here as its own line saying how many payments it stands in for, and status filters skip it. The archive is never read while paying: it is where a row goes to stay readable, not to keep being counted.
