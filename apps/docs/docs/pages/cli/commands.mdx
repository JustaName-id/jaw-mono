...
A spend on a token the session already meters over the same period is replaced rather than added to: `--x402 --limit 10/day` on a session already allowing 5 a day asks for 10, not 15. A different period is a different budget on chain, so it is kept as its own.
