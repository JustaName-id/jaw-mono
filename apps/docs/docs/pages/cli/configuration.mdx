...
| `x402`          | `object` | Spending limits for [x402 payments](/cli/x402)         | Yes, per field |
...
Spending limits for [x402 payments](/cli/x402). Set one field at a time:
...
Two more caps, `maxPerPeriod` and its `period`, are seeded from the on chain grant and mirror it. They are not settable, because they describe what the permission already enforces.

:::warning
These fields are not reachable from the MCP tools. An agent must not be able to raise its own spending caps, only a human at a terminal.
:::
