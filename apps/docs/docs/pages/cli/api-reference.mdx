...
| `wallet_getPermissions`    | No      | List permissions (includes expired and not-yet-started) |
...
```bash
jaw rpc call wallet_revokePermissions '{"id":"0x..."}'
```

`id` is the permission hash returned by `wallet_grantPermissions` and listed by `wallet_getPermissions`.
