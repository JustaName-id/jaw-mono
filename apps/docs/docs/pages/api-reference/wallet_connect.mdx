...
- If `signInWithEthereum` capability is requested:
  ...
  - On a fresh connect, the SDK fires a fire-and-forget usage report counting one signature per account that granted SIWE (address only — the SIWE message is never sent); cached reconnects are not re-reported
