...
| -------------- | ------------- | -------- | ----------------------------------------------------------------------- |
| `address`      | `0x${string}` | No       | Account address to sign with                                            |
...
- On success, the SDK fires a fire-and-forget usage report counting the signature for the workspace (signer address only — message content is never sent); unauthenticated calls without an address are not reported
