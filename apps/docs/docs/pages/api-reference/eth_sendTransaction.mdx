...
| --------- | ------------- | -------- | --------------------------------------------- |
| `from`    | `0x${string}` | No       | Account address to send from                  |
...
| -32602 | Invalid params — missing `to`, or a field that isn't a hex quantity  |

`to` is required: a smart account executes through `execute`, so contract
creation via `eth_sendTransaction` isn't supported. Params are validated in the
SDK before any wallet UI opens.
