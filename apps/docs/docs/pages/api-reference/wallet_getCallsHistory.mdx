...
| `transactions` | `array` | Transaction details (empty when pending) |
...
history.forEach((item) => {
  const status = item.status === 200 ? 'Success' :
                 item.status === 100 ? 'Pending' : 'Failed';
  const date = new Date(item.timestamp * 1000).toLocaleDateString();

  console.log(`${date}: ${status} - ${item.id}`);

  if (item.transactions.length > 0) {
    item.transactions.forEach((tx) => {
      console.log(`  Chain ${tx.chainId}: ${tx.transactionHash}`);
    });
  }
});
```
