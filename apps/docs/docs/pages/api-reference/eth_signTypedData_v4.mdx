...
await jaw.provider.request({
  ...
  params: ['0x1234...', JSON.stringify(typedData)],
});
```
...
- On success, the SDK fires a fire-and-forget usage report counting the signature for the workspace (signer address only — typed data content is never sent); reporting never blocks or fails the signing flow
