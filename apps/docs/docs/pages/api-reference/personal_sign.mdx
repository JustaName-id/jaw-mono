...
- On success, the SDK fires a fire-and-forget usage report counting the signature for the workspace (signer address only — message content is never sent); unauthenticated calls without an address are not reported, and reporting never blocks or fails the signing flow
