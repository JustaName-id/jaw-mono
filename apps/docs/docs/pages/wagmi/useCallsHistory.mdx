...
```typescript
import { useCallsHistory } from '@jaw.id/wagmi';
```
...
```typescript
function useCallsHistory(parameters?: {
  address?: Address;
  chainId?: number;
  connector?: Connector;
  index?: number;
...
function AddressHistory({ address }: { address: `0x${string}` }) {
  ...
  const { data: history, isLoading } = useCallsHistory({ address });
...
function PaginatedHistory({ address }: { address: `0x${string}` }) {
  ...
  const { data: history, isLoading, isFetching } = useCallsHistory({
    address,
    index: cursor,
    limit: 10,
    sort: 'desc',
...
function PendingTransactions() {
  const { data: history } = useCallsHistory();
...
function RefreshableHistory() {
  const { data, refetch, isFetching } = useCallsHistory();
