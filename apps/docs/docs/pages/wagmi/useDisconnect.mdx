...
```typescript
function useDisconnect(parameters?: { config?: Config; mutation?: UseMutationParameters }): UseMutationResult;
```
...
function DisconnectButton() {
  ...
  return <button onClick={() => disconnect({ connector })}>Disconnect JAW</button>;
}
```
...
function WalletButton() {
  ...
  return (
    <button onClick={() => connect({ connector: jaw({ apiKey: '...' }) })} disabled={isConnecting}>
