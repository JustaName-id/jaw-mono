...
In React Native, `window` and `navigator.credentials` are unavailable. Pass `Passkey.get` and `Passkey.create` from [`react-native-passkey`](https://github.com/f-23/react-native-passkey) in `config` — JAW handles all format conversion, challenge generation, and public key extraction internally. The casts bridge `react-native-passkey`'s literal string types to JAW's adapter types; the runtime shapes match.
...
import type { SyncStorage, NativePasskeyGetFn, NativePasskeyCreateFn } from '@jaw.id/core';
