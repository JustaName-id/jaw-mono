...
`apiKey` is optional on the type because whether a request is served is the backend's decision rather than the SDK's. Which methods work without one is a separate question, answered per method: the passkey flows are refused without a key, and `fromLocalAccount` is not. Each method's own page says which.
