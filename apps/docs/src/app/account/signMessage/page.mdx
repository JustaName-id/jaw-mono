...
```typescript
async function proveOwnership(account: Account, nonce: string) {
  const message = `Prove ownership of ${account.address}\nNonce: ${nonce}`;
...
```typescript
async function createAuthToken(account: Account) {
  const timestamp = Date.now();
  const message = `Authorize session\nTimestamp: ${timestamp}`;
...
Smart account signatures are wrapped according to ERC-6492 format for counterfactual accounts, ensuring they can be verified even before the account is deployed.
