...
```typescript
// Restore existing session (no WebAuthn prompt if already authenticated)
...
```typescript
// Get stored accounts and login with a specific one
const storedAccounts = Account.getStoredAccounts('your-api-key');
const account = await Account.get(
  { chainId: 1, apiKey: 'your-api-key' },
  storedAccounts[0].credentialId // Triggers WebAuthn
);
```
