...
<Callout type="info">
For React applications, use `JAWProvider` from `@jaw/ui` which provides a pre-built UIHandler implementation. See the [UI Package documentation](/ui-package) for details.
</Callout>

#### Using @jaw/ui (Recommended for React)

```typescript
import { JAWProvider } from '@jaw/ui';
