...
function PermissionsList() {
  ...
  const { data: permissions, isLoading } = Hooks.usePermissions();
...
```tsx
// Get permissions for a specific address
...
```tsx
const { data, refetch } = Hooks.usePermissions({
  query: {
...
- [account.getPermission()](/account/getPermission) - Account class method
