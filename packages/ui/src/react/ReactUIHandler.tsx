...
function PermissionDialogWrapper({
  ...
  const spends = useMemo(
    () =>
      spendsData.map((spend) => {
        const tokenInfo =
          tokenInfoMap[spend.token] ||
          (isNativeToken(spend.token)
            ? {
                decimals: viemChain?.nativeCurrency?.decimals ?? 18,
                symbol: nativeSymbol,
              }
    ...
    >
      <div className="flex flex-col gap-6 p-6">
        ...
        <div className="text-center">
          <h3 className="text-xl font-bold text-gray-900 mb-2">Unsupported Method</h3>
          <p className="text-sm text-gray-600 mb-4">This wallet does not support the following method:</p>
          <div className="bg-gray-100 rounded-lg p-4">
            <code className="text-sm font-mono text-gray-900 break-all">{method}</code>
          </div>
        </div>
