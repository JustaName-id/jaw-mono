...
/**
 * Shape only, so the refusal below can name the field that was wrong. Casing is
 * not checked here because it is not a reason to refuse: `normalized` fixes it.
 */
 ...
 * and viem checksums those at signing: an all-uppercase or mis-cased hex string
 * passes the regex above and then throws inside `sign()`. That throw lands
 * after the payer has been funded, so a server that merely cased an address
 * badly would cost the user a top-up and refuse the payment.
 *
 * Checksumming is a re-casing, and an `address` encodes lowercased either way,
 * so the signature is byte-identical to the one the raw string would have
 * produced. Only applied where the value is already known to match the regex.
 *
 * This deliberately accepts an EIP-55 checksum that is wrong, where viem would
 * refuse it. That refusal was never protection here, since an all-lowercase
 * string sails past it, and these values are machine-generated by the server:
 * the bytes are the bytes, whatever case they arrived in.
 */
const normalized = (value: `0x${string}`): `0x${string}` => getAddress(value);
...
export async function buildUptoPayment(
  ...
  const message: UptoPermitMessage = {
    ...
    nonce: BigInt(nonce),
    deadline,
    witness: { to: normalized(requirement.payTo), facilitator, validAfter },
  };
