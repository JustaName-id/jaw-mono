...
export async function toJustanAccount(
  ...
  return toSmartAccount({
    ...
    async getFactoryArgs() {
      ...
      // Skip factory args if account is already deployed
      try {
        ...
        // If RPC fails, conservatively include factory args
        console.warn(
          "[toJustanAccount] getCode check failed, including factory args:",
          error,
        );
      }
    ...
    async signAuthorization(
      executor: "self" | Account | Address | undefined = "self",
    ) {
      if (!isEip7702) {
        throw new BaseError(
