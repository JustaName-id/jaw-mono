...
describe("Account", () => {
  ...
  describe("buildChainConfig helper", () => {
    it("should build correct RPC URL with apiKey", () => {
      ...
      expect(expectedUrl).toBe(
        "https://api.justaname.id/proxy/v1/rpc?chainId=1&api-key=test-api-key",
      );
    });
    ...
    it("should build correct RPC URL without apiKey", () => {
      ...
      expect(expectedUrl).toBe(
        "https://api.justaname.id/proxy/v1/rpc?chainId=1",
      );
    });
  ...
  describe("Error handling", () => {
    it("get should throw when not authenticated and no credentialId provided", async () => {
      await expect(Account.get({ chainId: 1, apiKey: "test" })).rejects.toThrow(
        "Not authenticated",
      );
    });
  ...
  describe("sendCalls and getCallStatus", () => {
    it("sendCalls should store call status and wait for receipt in background", async () => {
      const { createSmartAccount, sendCalls: sendSmartAccountCalls } =
        await import("./smartAccount.js");
      const { storeCallStatus, waitForReceiptInBackground } =
        await import("../rpc/wallet_sendCalls.js");
      ...
      expect(waitForReceiptInBackground).toHaveBeenCalledWith(
        mockUserOpHash,
        1,
        "test-api-key",
      );
    ...
    it("sendCalls with permissionId should use sendCallsWithPermission", async () => {
      const { createSmartAccount, sendCallsWithPermission } =
        await import("./smartAccount.js");
      const { storeCallStatus, waitForReceiptInBackground } =
        await import("../rpc/wallet_sendCalls.js");
      ...
      const mockUserOpHash =
        "0xpermission1234567890abcdef1234567890abcdef1234567890abcdef12345678";
    ...
    it("sendCalls without permissionId should use standard sendCalls", async () => {
      ...
      const mockUserOpHash =
        "0xstandard1234567890abcdef1234567890abcdef1234567890abcdef123456789";
      vi.mocked(sendSmartAccountCalls).mockResolvedValue({
        id: mockUserOpHash,
        chainId: 1,
      });
  ...
  describe("restore", () => {
    ...
    it("should throw when credentialId is empty", async () => {
      await expect(
        Account.restore({ chainId: 1, apiKey: "test" }, "", "0x04abc123"),
    ...
    it("should restore account without options", async () => {
      const { toWebAuthnAccount } = await import("viem/account-abstraction");
      const { createSmartAccount } = await import("./smartAccount.js");
      const mockSmartAccount = {
        address: "0x1234567890123456789012345678901234567890",
    ...
    it("should forward only getFn when rpId is not provided", async () => {
      const { toWebAuthnAccount } = await import("viem/account-abstraction");
      const { createSmartAccount } = await import("./smartAccount.js");
      const mockSmartAccount = {
        address: "0x1234567890123456789012345678901234567890",
