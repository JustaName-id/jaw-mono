  ...
  implements ProviderInterface
{
  ...
  constructor({
    ...
    const expectedSignerType: SignerType =
      preference.mode === Mode.AppSpecific ? "appSpecific" : "crossPlatform";
      ...
      if (!this.signer) {
        switch (args.method) {
          ...
          case "wallet_getCallsHistory": {
            const result = await handleGetCallsHistoryRequest(
              args,
              this.apiKey,
            );
            return result as T;
          ...
          case "eth_chainId": {
            const result = hexStringFromNumber(
              this.metadata.defaultChainId ?? 1,
            ) as T;
            return result;
          }
          default: {
  ...
  private initSigner(signerType: SignerType): Signer {
    return createSigner({
      ...
      communicator:
        signerType === "crossPlatform" ? this.communicator : undefined,
      uiHandler:
        signerType === "appSpecific" ? this.preference.uiHandler : undefined,
