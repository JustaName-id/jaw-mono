...
export class CrossPlatformSigner extends JAWSigner {
  ...
  constructor(params: ConstructorOptions) {
    ...
    // Initialize adapter if it supports init (mobile adapters)
    this.adapter.init?.({
  ...
  override async handshake(args: RequestArguments): Promise<void> {
    ...
    // Wait for the adapter to be ready (e.g. popup loaded, deep link opened)
    ...
    // Handle unencrypted response (browser mode / mobile adapters)
    ...
    // Encrypted flow: store peer's public key
  ...
  protected override async handleWalletConnect(
  ...
  protected override async handleWalletConnectUnauthenticated(
    request: RequestArguments,
  ): Promise<unknown> {
    // Wait for the adapter to be ready
    await this.adapter.waitForReady();
   ...
   */
  private resolveChainFromRequest(
    request: RequestArguments,
  ): SDKChain | undefined {
  ...
  private async sendUnencryptedRequest(
    request: RequestArguments,
    overrideChain?: SDKChain,
  ): Promise<unknown> {
    let chain: SDKChain;
    ...
    const response: RPCResponseMessage =
      await this.adapter.postRequestAndWaitForResponse(message);
    ...
    throw standardErrors.rpc.internal(
      "Expected unencrypted response in browser mode",
    );
  }
  ...
  private async decryptResponseMessage(
    ...
    // Guard: unencrypted content should not reach decrypt path
    if ("unencrypted" in content) {
      throw standardErrors.rpc.internal(
        "Received unencrypted response in encrypted flow",
      );
    }

    // Guard: response must contain encrypted content
    if (!("encrypted" in content)) {
      throw standardErrors.rpc.internal(
        "Invalid response format: expected encrypted content",
      );
    }
