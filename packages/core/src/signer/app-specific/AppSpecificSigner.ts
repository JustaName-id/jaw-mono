...
export class AppSpecificSigner extends JAWSigner {
  ...
  protected async handleWalletConnect(
  ...
  protected async handleWalletConnectUnauthenticated(
    request: RequestArguments,
  ): Promise<unknown> {
  ...
  private async performWalletConnect(
    ...
    const chain =
      chains?.find((c: SDKChain) => c.id === this.chain.id) ?? this.chain;

    // Extract capabilities and silent flag from request params
    const walletConnectParams = modifiedRequest.params as
    ...
    const response =
      await this.uiHandler.request<WalletConnectResponse>(uiRequest);
    ...
    if (!response.data) {
      throw UIError.userRejected(
        "Invalid wallet_connect response: missing data",
      );
    }
  ...
  protected async handleSigningRequest(
    ...
    switch (request.method) {
      ...
      case "wallet_sendCalls": {
        ...
        type WalletSendCallsParams = Omit<
          TransactionUIRequest["data"],
          "chainId"
        > & {
          /** Target chain ID in hex format. Defaults to the connected chain. */
          chainId?: `0x${string}`;
      ...
      case "eth_sendTransaction": {
        type EthSendTransactionParams = Omit<
          SendTransactionUIRequest["data"],
          "chainId" | "from"
        > & {
          from?: Address;
          /** Target chain ID in hex format. Defaults to the connected chain. */
      ...
      case "wallet_grantPermissions": {
        const grantParams =
          request.params as WalletGrantPermissionsRequest["params"];
      ...
      case "wallet_revokePermissions": {
        const revokeParams =
          request.params as WalletRevokePermissionsRequest["params"];
