...
import { SiweModal } from '../../components/SignatureModal';
...
interface SiweModalWrapperProps {
...
export const SiweModalWrapper: React.FC<SiweModalWrapperProps> = ({
  request,
  config,
  onApprove,
  onReject,
  ...
  const appName = extractAppName(displayMessage);
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      onReject(error instanceof Error ? error : new Error('Failed to sign in'));
    } finally {
      setIsSigning(false);
    }
  };
